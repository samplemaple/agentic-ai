#!/bin/bash
# ============================================================
# OpenClaw 部署脚本（Step 3-6）
# 跳过 Step 1（系统初始化）和 Step 2（Node.js / OpenClaw 安装）
#
# 前置条件：
#   - 已完成系统更新（apt update && apt upgrade）
#   - 已安装 Node.js 24+ 和 openclaw
#   - 准备好大模型 API Key
#
# 使用方法：
#   chmod +x setup-openclaw-step3.sh
#   sudo ./setup-openclaw-step3.sh
# ============================================================

set -e

echo "=========================================="
echo "  OpenClaw 部署脚本（Step 3-6）"
echo "=========================================="
echo ""

# 前置检查
if ! command -v node &> /dev/null; then
    echo "❌ 未检测到 Node.js，请先完成 Step 1-2 或手动安装 Node.js 24+"
    exit 1
fi

if ! command -v openclaw &> /dev/null; then
    echo "❌ 未检测到 openclaw，请先运行：npm install -g openclaw@latest"
    exit 1
fi

echo "检测到 Node.js: $(node -v)，OpenClaw 已安装，继续部署..."
echo ""
...
echo ""
echo "6.2  验证 Tailscale SSH（在你的本地终端执行）："
